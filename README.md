...
Given more time, I would have preferred to convert those nine rules into a properly recursive parser, capable of accommodating logical rules of potentially infinite complexity.  In such a model, there would, in theory, be no limit to the number of expansions/transformations produced by the Context-Free Grammar.  In other words, there would be limits neither to the number of branches within the grammatical tree nor to the levels of depth within it.  Recursive structures could be nested over and over again.  
