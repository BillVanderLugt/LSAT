...
Those 9 CFG transformations are nevertheless capable of generating an infinite variety of logical/grammatical structures.  And, given more time, I would have preferred to implement those 9 rules within a properly recursive parser, capable of nesting structures many layers deep.  In such a model, there would, in theory, be no limit to the variety of structures producable by the CFG.  In other words, there would be limits neither to the number of branches within the grammatical tree nor to the levels of depth within it.  Armed with full recursive capabilities, a machine parser could likely outperform humans on complicated structures.
